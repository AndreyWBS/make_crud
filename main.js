...
async function main() {
    ...
    engine
        ...
        .addGenerator(new LayerGenerator(targetDir, 'middlewares/validators', validatorTemplates.validator))
        .addGenerator(new LayerGenerator(targetDir, 'routes', crudTemplates.routes))
        .addGenerator(new LayerGenerator(targetDir, 'docs', crudTemplates.documentation , 'md'))
        .addGenerator(new LayerGenerator(targetDir, 'docs/html', crudTemplates.documentationHtml, 'html'))
        .addGenerator(new LayerGenerator(targetDir_cli, 'apiClient', crudTemplates.apiClient));

    engine
        .addGenerator(new StaticFileGenerator(targetDir, 'src/config/database.js', infraTemplates.database))
        .addGenerator(new StaticFileGenerator(targetDir, 'src/config/env.js', infraTemplates.env))
        .addGenerator(new StaticFileGenerator(targetDir, 'src/middlewares/authMiddleware.js', infraTemplates.authMiddleware))
        .addGenerator(new StaticFileGenerator(targetDir, 'src/middlewares/errorMiddleware.js', infraTemplates.errorMiddleware))
        .addGenerator(new StaticFileGenerator(targetDir, 'src/docs/html/index.html', infraTemplates.indexDocumentation))
